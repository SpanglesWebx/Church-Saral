...
const addStudentAuction = async (req, res) => {
  try {
    ...
    // 🔴 Prevent duplicate auction same student same item same day
    const existing = await StudentAuction.findOne({
      "seller.member_id": seller.member_id,
      item,
      date: new Date(date),
    });
    ...
    if (existing) {
      return res.status(400).json({
        message: "Auction already exists for this student and item today",
      });
    }

    const auction = new StudentAuction({
      date,
      seller,
      buyer,
      item,
    ...
    console.error("Add Student Auction Error:", err);

    return res.status(500).json({
      message: "Server error while adding auction",
    });
  }
};
