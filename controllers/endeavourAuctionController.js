...
const addEndeavourAuction = async (req, res) => {
  try {
    ...
    const existing = await EndeavourAuction.findOne({
    ...
    if (existing) {
      return res.status(400).json({
        message: "Auction already exists for this seller and item on this date",
      });
    }

    // ================= CREATE AUCTION =================

    const auction = new EndeavourAuction({
      date,
      seller,
      buyer,
      item: item.trim(),
    ...
    console.error("Add Endeavour Auction Error:", err);
