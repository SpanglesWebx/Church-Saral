...
export const SundaySclPaidExpense = () => {
...
const fetchExpenses = async () => {
  try {
    const res = await axios.get(`${URL}/sundayschool-expenses`, {
      ...
      params: {
        page: CurrentPage,
        limit: 10,
        status: "Paid",  // you already fixed Paid in this component
  ...
  // const fetchExpenses = async () => {
  //   try {
  //     const res = await axios.get(`${URL}/sundayschool-expenses?status=Paid`, {
  //       headers: { Authorization: token },
  //     });
  //     setExpenses(res.data.expenses || []);
  //     setTotalPages(res.data.totalPages || 1);
...
useEffect(() => {
  ...
  const handleCloseBill = async () => {
    ...
    try {
      await axios.patch(
        `${URL}/sundayschool-expenses/close-bill/${selectedExpense._id}`,
        formData,
        { headers: { Authorization: token } }
      );
