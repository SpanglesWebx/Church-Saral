...
export const SundaySclExam = () => {
  ...
  const [editTargetClass, setEditTargetClass] = useState(null); // the class object being edited (from selectedExam.classExams)
  const [editClassStudents, setEditClassStudents] = useState([]); // list of candidate students to show in modal (we'll use existing participants)
  const [editSelectedStudentIds, setEditSelectedStudentIds] = useState([]); // member_id strings
  ...
  const handleAddEvent = () => navigate("/admin/exam/addsundayschoolexam");

  const fetchExams = async () => {
    try {
      const res = await axios.get(`${URL}/sundayschool-exams/all`, {
  ...
  /**
   * Open Edit Participants modal
   * We DO NOT call the /class/:className/students endpoint (it returned 404 for you).
   * Instead we present the current participants (so admin can toggle who should remain).
   * If you later add a valid endpoint for listing all students in a class, you can merge that list here.
   */
  const handleEditParticipants = (cls) => {
    if (!selectedExam) {
      setResponse({ status: "Failed", message: "Please open an exam first (view) before editing participants." });
      return;
    }

    try {
      // cls is the class object from selectedExam.classExams
      setEditTargetClass(cls);

      // Use the existing participants as the base list for checkboxes
      ...
      // Pre-check those who already are participants
      setEditSelectedStudentIds(currentParticipants.map(p => p.member_id));

      setIsEditParticipantsOpen(true);
    } catch (err) {
      console.error("Open edit participants error:", err);
      setResponse({ status: "Failed", message: "Failed to open participants editor" });
    }
  };
  ...
  /**
   * Save edited participants:
   * - Send the participants array (member objects) to /add-participants
   * - Update local `exams` and `selectedExam` without needing a full page reload
   */
  const saveEditedParticipants = async () => {
    try {
      ...
      // Build selected student objects from editClassStudents
      // (only include those whose member_id is checked)
  ...
  section_name: s.section_name
      ...
      // Update local state immediately so UI reflects changes without full reload
      ...
      // Also update selectedExam if it's the same exam
  ...
  const saveEditedMarks = async (className) => {
    try {
      ...
      // Refresh the single exam locally from server response by re-fetching exams page
      // (you could also update state locally similar to participants)
      const updatedRes = await axios.get(`${URL}/sundayschool-exams/all`, {
        headers: { Authorization: token },
        params: {
          page: currentPage,
          limit: rowsPerPage,
  ...
  return (
    <>
        ...
        <div className="overflow-x-auto mt-4">
          <table className="w-full text-sm text-gray-500">
            ...
            <tbody>
              {exams.length > 0 ? (
                exams.map((exam, idx) => (
                  <tr key={exam._id} className="p-2 text-center">
                    ...
                    <td className='p-2 text-center'>
                      <div className="flex items-center justify-center gap-3">
                        ...
                        <CiEdit title='Edit exams' size={20} className="text-lavender--600 cursor-pointer" onClick={() => navigate(`/admin/exam/editsundaysclexam/${exam._id}`)} />
                      </div>
                    </td>
                  </tr>
                ))
      ...
      <Modal isOpen={isViewModalOpen} onClose={() => setIsViewModalOpen(false)} title="View Exam Details">
        {selectedExam ? (
          <div className="text-sm text-gray-700 space-y-4 max-h-[500px] overflow-y-auto">
            {[
                  ...
                  <tbody>
                    {selectedExam.teacherDetails.map((td, i) => (
                      <tr key={td._id || i}>
                        ...
                        <td className="p-2 border">{td.teacher?.member_name}</td>
                        <td className="p-2 border text-center">{td.teacher?.member_id}</td>
                  ...
                  <div key={cls._id || idx} className="mt-3 border p-3 rounded">
                    <div className="flex items-start justify-between mb-2">
                      ...
                      {/* <div className="flex items-center gap-2">
                        <button className="px-2 py-1 border rounded text-sm text-white bg-lavender--600" onClick={() => handleEditParticipants(cls)} title="Edit Participants">Edit Participants</button>

                        <button className={`px-2 py-1 border rounded text-sm ${participants.length === 0 ? "opacity-50 cursor-not-allowed" : "text-white bg-lavender--600"}`} onClick={() => participants.length > 0 && handleEditMarksToggle(cls)} title="Edit Marks" disabled={participants.length === 0}>Edit Marks</button>
                      </div> */}
                            ...
                            return (
                              <tr key={p._id || p.member_id || i}>
                                ...
                                <td className="p-2 border text-center">
                                  {isEditingThisClass ? (
                                    <input type="number" min={0} className="border rounded px-2 py-1 w-20 text-center" value={draft?.marks ?? ""} onChange={(e) => handleDraftMarkChange(cls.className, i, e.target.value)} />
                                  ) : (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
            {(editClassStudents || []).map((stu) => (
              <label key={stu._id || stu.member_id} className="flex items-center space-x-2">
                <input type="checkbox" checked={editSelectedStudentIds.includes(stu.member_id)} onChange={() => toggleEditStudentCheckbox(stu.member_id)} />
                <span>{stu.member_name} ({stu.member_id})</span>
              </label>
            ))}
