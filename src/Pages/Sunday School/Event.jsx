...
export const Event = () => {
  ...
  const fetchEvents = async () => {
    try {
      const res = await axios.get(`${URL}/sundayschool-events`, {
        params: {
          search,
          startDate,
          endDate,
  ...
  const fetchEventById = async (id) => {
    ...
    } catch (err) { console.error("Error fetching event by id:", err); }
  };
  ...
  const handleAddEvent = () => navigate("/admin/event/addsundayschoolevent");
  const handleEditEvent = (event) => navigate(`/admin/event/editevent/${event._id}`, { state: { event } });
              ...
              {events.length > 0 ? (
                events.map((event, index) => (
                  <tr key={event._id} className="border-b ">
                    <td className="p-2 text-center">{(currentPage - 1) * (rowsPerPage || 0) + index + 1}</td>
          ...
          <div className="text-sm text-gray-700 space-y-3 max-h-[550px] overflow-y-auto">
                      ...
                      {hasParticipants ? (
                        <ul className="mt-1 text-gray-700">
                          {comp.participants.map((p, idx) => (
                            <li key={p._id}>
                              {p.member?.member_name ?? p.member_name ?? "Unknown"}
            ...
            {(selectedEvent.teacherCompEvents || []).length > 0 ? (
              (selectedEvent.teacherCompEvents || []).map((comp) => {
                ...
                const hasAnyPrize = participants.some(
                  (p) => p.prize && p.prize !== "None"
                );
                  ...
                  <div key={comp._id} className="mt-2 border rounded p-2 bg-gray-50">
                    ...
                    {hasParticipants ? (
                      <ul className="mt-1 text-gray-700">
                        {comp.participants.map((p, idx) => (
                          <li key={idx}>
                            {p.member?.member_name || p.member_name}
        ...
        {selectedCompetition && (
          <div className="space-y-4">
            ...
            <div className="mt-2 space-y-1">
              {selectedCompetition.competition.participants.map((p) => {
                const memberId = p.member?.member_id || p.member_id;
                const memberName = p.member?.member_name || p.member_name;
                        ...
                        setSelectedCompetition((prev) => ({
                          ...
                          competition: {
                            ...
                            participants: prev.competition.participants.map((st) => {
                              const stId = st.member?.member_id || st.member_id;

                              return stId === memberId
            ...
            <div className="flex justify-end gap-2 mt-4">
              ...
              <button
                ...
                onClick={async () => {
                  const updatedParticipants = selectedCompetition.competition.participants.filter(
                    (p) => p.selected !== false
                  );
                      ...
                      const res = await axios.put(
                        `${URL}/sundayschool-events/update-teacher-participants`,
                        {
                          ...
                          participants: updatedParticipants.map(p => ({
                            member_id: p.member?.member_id || p.member_id,
                            member_name: p.member?.member_name || p.member_name,
                            prize: p.prize || "",
                          }))
                          ,
                        },
