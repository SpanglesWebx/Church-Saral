...
export const Studentauction = () => {
  ...
  // ===== Fetch Auctions (same behavior as Auction.jsx) =====
  const fetchAuctions = async (page = 1, query = "") => {
    try {
      const res = await axios.get(`${URL}/student-auctions`, {
        params: { page, limit: rowsPerPage, query },
        headers: { Authorization: token },
      });

      const formatted = res.data.auctions.map((auction, index) => ({
        ...
        buyerId: auction.buyer?.member_id || "Non-member",
  ...
  const onSubmit = async (formData) => {
    ...
    try {
      ...
      // 🔴 Manual required validation for seller
      if (!selectedClass || !selectedStudent) {
        setResponse({
          status: "Failed",
          message: "Please select Class and Student",
        });
        setSaving(false);
        return;
      }
      ...
      await axios.post(`${URL}/student-auctions`, payload, {
        headers: { Authorization: token },
      });
  ...
  const handleOpenModal = () => {
    reset({ date: new Date().toISOString().split("T")[0], payment_status: "Unpaid" });
    setSelectedStudent(null);
    setSelectedClass(null);
    setBuyerSearch("");
    setBuyerIdSearch("");
  ...
  const handleCloseModal = () => {
    ...
    // clear dropdowns and selected data
    setSelectedStudent(null);
    setSelectedClass(null);
    setStudentOptions([]);
    setBuyerDropdown([]);
  ...
  const handleClassChange = async (classId) => {
    try {
      const res = await axios.get(`${URL}/sunday-classes/${classId}/details`, {
        headers: { Authorization: token },
      });
    ...
    <div className="p-4">
      ...
      <div className="h-full p-4 mx-1 mt-3 bg-white rounded-xl">
        ...
        <div className="overflow-x-auto mt-4">
          {loading ? (
            <p className="p-4 text-center text-gray-500">Loading auctions...</p>
          ) : auctions.length === 0 ? (
            <p className="p-4 text-center text-gray-500">No auctions found</p>
          ) : (
                      ...
                      >
                        {col.key === "action" ? (
                          <div className="flex justify-center">
                            <FaEye
                              size={18}
                              className="cursor-pointer text-lavender--600  transition"
                              onClick={() => {
