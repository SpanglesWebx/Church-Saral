...
export const MenPaidExpense = () => {
  ...
  const fetchExpenses = async () => {
    try {
      const res = await axios.get(`${URL}/menfellow-expenses`, { // updated Men Fellowship endpoint
  ...
  const handleCloseBill = async () => {
    ...
    try {
      await axios.patch(
        `${URL}/menfellow-expenses/close-bill/${selectedExpense._id}`,
        formData,
        { headers: { Authorization: token } }
      );
