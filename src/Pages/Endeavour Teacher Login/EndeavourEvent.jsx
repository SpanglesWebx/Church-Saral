...
export const EndeavourEvent = () => {
  ...
  const fetchEvents = async () => {
    try {
      ...
      const res = await axios.get(`${URL}/endeavour-events/teacher/${encodeURIComponent(teacherId)}`, {
  ...
  const enrollTeacher = async () => {
    try {
      ...
      const updated = await axios.get(`${URL}/endeavour-events/${selectedEvent._id}`, {
        headers: { Authorization: token },
      });

      if (updated.data?.event) {
        setSelectedEvent(updated.data.event);
      }
          ...
          <div className="text-sm text-gray-700 space-y-3 max-h-[580px] overflow-y-auto">
            ...
            {[
              ...
              { label: "Description", value: selectedEvent.description },
              {
                ...
                value: selectedEvent.teacherCompetitions || [],
                hasButton: true
              },
            ].map((item, i) => (
              <div key={i} className="grid grid-cols-12 gap-2 py-2">

                <div className="col-span-12 sm:col-span-4 font-semibold">
                  {item.label}
                </div>
                ...
                <div className="col-span-12 sm:col-span-8">
                  ...
                  {item.hasButton ? (
                    ...
                    item.value.map((comp) => {
                      ...
                      const enrolled = selectedEvent.teacherCompEvents?.some(
                        (t) =>
                          t.competition === comp &&
                          t.participants?.some(
                            (p) => String(p.member?.member_id) === String(teacherId)
                          )
                      );

                      return (
                        <div key={comp} className="grid grid-cols-12 items-center mb-1">
            ...
            <h3 className="font-semibold text-base mt-4">Teacher Participants</h3>

            {selectedEvent.teacherCompEvents?.map((tc) => (
              <div key={tc._id} className="border p-2 rounded mt-2">

                <p className="font-semibold">{tc.competition}</p>

                {tc.participants?.length ? (
                  <ul className="list-disc ml-5">

                    {tc.participants.map((p) => (
                      <li key={p._id}>
              ...
              onClick={() => {
                ...
                const preselected = (students || []).filter((stu) =>
                  (selectedCompetition.participants || []).some(
                    (p) => String(p.member?.member_id) === String(stu.member_id)
                  )
                );
      ...
      >
        <div className="space-y-2">
          {selectedEvent?.teacherCompetitions?.map((comp, idx) => {
            ...
            const alreadyEnrolled = selectedEvent.teacherCompEvents?.some(
              (t) =>
                t.competition === comp &&
                t.participants?.some(
                  (p) => String(p.member?.member_id) === String(teacherId)
            ...
            return (
              <div key={idx} className="flex items-center">
                ...
                <input
                  ...
                  checked={alreadyEnrolled || selectedTeachers.includes(comp)}

                  disabled={alreadyEnrolled}   // prevent changing existing enrollment
