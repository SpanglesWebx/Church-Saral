...
export const WomenFellowEvent = () => {
  ...
  const fetchWomenMembers = async () => {
    try {
      ...
      const res = await axios.get(
        `${URL}/womens-fellowship`,
        { headers: { Authorization: token } }
      );

      const members = res.data?.data || [];

      setWomenMembers(members);
  ...
  const saveParticipants = async () => {
    ...
    try {
      ...
      const res = await axios.post(
        `${URL}/women-events/participants/add`,
        payload,
        { headers: { Authorization: token } }
      );
  ...
  const handleSaveEditParticipants = async () => {
    ...
    try {
      ...
      const res = await axios.put(
        `${URL}/women-events/participants/update`,
        {
  ...
  const onChangeAddCompetition = (compId) => {
    ...
    const comp = selectedEvent?.womenCompetitions?.find(
