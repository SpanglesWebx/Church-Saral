...
export const WomenAuctionReport = () => {
    ...
    const fetchAuctions = async () => {
      try {
        const res = await axios.get(
          `${URL}/women-auctions/report/by-buyer?search=${search}&page=${CurrentPage}&limit=${rowsPerPage}`,
  ...
  const handlePayment = async (auctionId) => {
    ...
    try {
      ...
      // ✅ Refresh report
      const res = await axios.get(
        `${URL}/women-auctions/report/by-buyer`,
        {
          headers: { Authorization: token },
          params: {
            page: CurrentPage,
            limit: rowsPerPage, // 🔥 FIX (no hardcode)
            search,
          },
        }
      );
            ...
            <tbody className="text-center">
              {auctions.length === 0 ? (
                ...
                auctions.map((buyer, bi) => (
                  <tr key={buyer.buyerId || bi} className="border-b">
                    ...
                    {/* Phone */}
                    <td className="p-2">
                      {buyer.buyerPhone ? buyer.buyerPhone : "-"}
                    </td>

                    {/* Overall Due */}
                    <td
                      className={`p-2 font-semibold ${buyer.overallUnpaid > 0
                        ? "text-red-600"
                        : "text-green-600"
                        }`}
                    >
            ...
            <div className="bg-gray-50 border border-gray-200 rounded-lg p-5 mb-3">
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 text-lg">
                ...
                <div className="flex items-center">
                  ...
                  <span className="text-gray-900">{selectedBuyer.buyerPhone || "-"}</span>
                </div>

                {/* Due Amount */}
                <div className="flex items-center">
                  <span className="font-semibold text-gray-700 w-[110px]">Due Amount:</span>
