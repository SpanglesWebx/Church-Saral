...
export const AddWomenEvent = () => {
  ...
  const validateForm = () => {
    ...
    const register = registerBefore ? new Date(registerBefore) : null;

    // Event date cannot be past
    if (event < today) {
      setResponse({
        status: "Failed",
    ...
    // Register before must be earlier than event
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setResponse({
        status: "Success",
        message: "Women's Fellow Event Added Successfully"
      });

      setTimeout(() => {
        navigate("/admin/womenfellowevent");
      }, 900);
      ...
      <div className={`${saving ? "pointer-events-none opacity-60" : ""}`}>
        ...
        <div className="flex justify-start mt-6">
          <FaArrowLeft
            ...
            onClick={() => navigate("/admin/womenfellowevent")}
          />
        </div>
