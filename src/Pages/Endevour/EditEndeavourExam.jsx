    ...
    const fetchData = async () => {
      ...
      try {
        ...
        const [examRes, classRes, examByRes] = await Promise.all([
          axios.get(`${URL}/endeavour-exams/${id}`, {
            headers: { Authorization: token }
          }),

          axios.get(`${URL}/endeavour-classes/event/groups`, {
            headers: { Authorization: token }
          }),

          axios.get(`${URL}/endeavour-exams/examby/all`, {
            headers: { Authorization: token }
          })
        ]);
        ...
        if (Array.isArray(exam.examBy) && exam.examBy.length) {
          const examById = exam.examBy[0]?._id || exam.examBy[0];
          setSelectedExamBy(examById);
        }
