...
export const EditBookingHall = () => {
  ...
  useEffect(() => {
    const fetchBooking = async () => {
      try {
        ...
        // ✅ set values
        setSelectedHallId(data.hall?._id || "");
        setSelectedCategoryId(data.category?._id || "");
        setCalendarDate(dayjs(data.date).toDate());
        setCustomerName(data.customerName || "");
        setCustomerPhone(data.customerPhone || "");
        setAdvanceAmount(data.advanceAmount || 0);
        setBookingStatus(data.booking_status || "Reserved");
        setAmount(data.amount || 0);

        // ✅ sessions
        setSelected({
          morning: data.sessions?.includes("morning"),
          evening: data.sessions?.includes("evening"),
        });
  ...
  const handleBooking = async () => {
    ...
    try {
      ...
      const res = await axios.put(`${URL}/marriage/bookings/${id}`, payload, {
        headers: { Authorization: token },
      });
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 items-end mt-3">
            ...
            <div>
              ...
              <div className="w-full mt-1 flex justify-center">
                <div className="inline-flex bg-gray-100 p-1 rounded-full shadow-sm gap-1">

                  {/* Morning */}
                  <button
                    disabled={!morningAvailable}
                    onClick={() => toggle("morning")}
                    className={`px-12 py-1 text-sm font-medium rounded-full transition-all
    ${selected.morning
  ...
  `}
          ...
          <div className="w-full mt-4 flex justify-center">
            ...
            <div className="w-full">
              ...
              <Calendar
                ...
                setSelectedSessions={setSelected}
                editingId={id}
                onSelect={(date) => {
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-2 gap-3 items-end mt-4">
            ...
            <div>
              ...
              <input
                ...
                value={advanceAmount}
                readOnly
                className="block w-full mt-1 rounded-md shadow-sm sm:text-sm bg-gray-100 border-gray-300 cursor-not-allowed"
              />
