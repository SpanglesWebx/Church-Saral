 ...
 const URL = import.meta.env.VITE_BACKEND_API_URL;
  ...
  const fetchData = async () => {
    try {
      ...
      const response = await axios.get(`${URL}/reports/baptism`, {
        params: {
      ...
      setData(response.data.Baptism);
      setFilteredData(response.data.Baptism);
    }catch (error) {
...
return fetchData();
  ...
  //     didDrawPage: (data) => {
  //       addTitle(doc, "Baptism Reports"); // Ensure title is added before table
  //     },
  //     styles: {
  ...
  try {
    ...
    const worksheet = workbook.addWorksheet("Baptism Report", {
      pageSetup: {
        paperSize: 9, // A4
    ...
    // Title row
    const titleRow = worksheet.addRow(["Baptism Reports"]);
    titleRow.font = { size: 14, bold: true };
    titleRow.alignment = { horizontal: "center" };
    const totalCols = showTamilOnly ? 6 : 7;
    const mergeEndCol = String.fromCharCode(64 + totalCols); // 'F' or 'G'
    worksheet.mergeCells(`A${titleRow.number}:${mergeEndCol}${titleRow.number}`);
    ...
    filteredData.forEach((item, index) => {
      const rowData = [
        ...
        item.secondary_family_id || item.primary_family_id,
        item.status
      ].filter(val => val !== null); // Remove member_name if hidden
    ...
    saveAs(blob, "BaptismReports.xlsx");
    ...
    <div className="relative h-auto ml-5 w-[100%] bg-gray-100">
      <div className="p-6 bg-white rounded-lg shadow-lg">
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-2xl font-semibold text-lavender--600">BaptismReports</h2>
          <FormControlLabel
            control={
              <Checkbox
          ...
          <div className="flex gap-x-5">
            <button
              ...
              className="mr-4 text-lavender--600 cursor-pointer hover:text-lavender--600"
            >
             <img src={down}/>
            </button>
            ...
            <thead>
              <tr>
                {tableHeading.filter((heading) => !(showTamilOnly && heading === "Member Name")).map(heading => (
                  <th key={heading} className="px-4 text-center py-2 text-base text-gray-700 bg-white border-b dark:bg-gray-700 dark:text-gray-400">{heading}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {currentPageData.map((item, index) => (
                <tr key={index} className="border-t border-gray-200">
                  ...
                  <td className="px-4 py-2 text-sm text-center">{moment(item.baptized_date).format('DD-MM-YYYY')}</td>
