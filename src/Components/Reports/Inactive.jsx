...
const handleDownloadExcel = async () => {
  try {
    ...
    const worksheet = workbook.addWorksheet("Inactive Report", {
      pageSetup: {
        paperSize: 9,
    ...
    // Title row
    const titleRow = worksheet.addRow(["Inactive Reports"]);
    titleRow.font = { size: 14, bold: true };
    titleRow.alignment = { horizontal: "center" };
    const totalCols = showTamilOnly ? 7 : 8;
    const mergeEnd = String.fromCharCode(64 + totalCols); // A-H or A-G
    ...
    saveAs(blob, "InactiveReports.xlsx");
            ...
            <thead>
              <tr>
                {tableHeading.filter((heading) => !(showTamilOnly && heading === "Member Name")).map((heading) => (
                  <th
                    key={heading}
                    className="px-4 py-2 text-center border-b text-base text-gray-700 bg-white dark:bg-gray-700 dark:text-gray-400"
                  >
                    {heading}
                  </th>
                ))}
